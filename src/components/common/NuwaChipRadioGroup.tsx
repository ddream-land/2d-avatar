...
const CustomRadio = (props: any) => {
  const { children, isSelected, isFocusVisible, getBaseProps, getLabelProps, getInputProps } = useRadio({
    ...props,
  });
...
export default function NuwaChipRadioGroup({
  items,
  value,
  onChange,
}: {
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      ...
      >
        {items.map((item) => (
          <CustomRadio key={item.value} value={item.value} onChange={onChange}>
            {item.label}
          </CustomRadio>
        ))}
      </RadioGroup>

...
import { uploadFileToServer } from "@/lib/common.api";
import { Progress, Spinner } from "@nextui-org/react";
import { customAlphabet } from "nanoid";
import { uploadModelFile } from "@/lib/voice.api";
        ...
        <>
          <div className="w-full h-full px-6 bg-zinc-800 rounded-xl justify-between items-center gap-3 inline-flex">
            <div className="justify-start items-center gap-3 flex text-slate-100 text-base font-normal leading-normal">
              {fileNameIcon && <div className="w-6 h-6 justify-center items-center flex">{fileNameIcon}</div>}
              <div className="text-slate-100 text-base font-normal leading-normal">{fileNameStr}</div>
            </div>
            {isUploading ? (
              <Spinner />
            ) : (
          ...
          <div className="w-full absolute bottom-0 left-0">
            {loaded !== total && <Progress size="sm" aria-label="Loading..." value={loaded} maxValue={total} />}
          </div>
        </>
      ) : (
          ...
          <Dropzone onDrop={onDropHander} accept={dropzoneAccept}>
            {({ getRootProps, getInputProps }) => (
              <section className="w-full h-full">
                <div
                  className="cursor-pointer w-full h-full p-4 rounded-2xl border-dashed border-2 border-zinc-700 flex-col justify-center items-center gap-2 inline-flex"
                  {...getRootProps()}
                >
                  <input {...getInputProps()} />
                  {icon}
                  <div className="text-center text-zinc-400 text-xs font-medium ">{label}</div>
                </div>
              </section>
            )}
          </Dropzone>

    ...
    <>
      <div className="w-full h-full relative" onClick={onOpen}>
        <div className="cursor-pointer w-full h-full p-4 rounded-2xl border-dashed border-2 border-zinc-700 flex-col justify-center items-center gap-2 inline-flex">
          ...
          <div className="text-center text-zinc-400 text-xs font-medium ">Select From Training Audio File</div>
        </div>
      </div>
            ...
            <>
                  ...
                  {opToneList.map((toneItem, index) => (
                    <ToneVoiceFile
                      ...
                      onTextChange={(newText) => {
                        setOpToneList(
                          opToneList.map((toneListItem, toneIndex) => {
                            if (index === toneIndex) {
                              return {
                                ...toneListItem,
                                tone: {
              ...
              <ModalFooter>
                <Button
                  ...
                  variant="ghost"
                  onPress={() => {
                    const selectedToneList = opToneList
                      .filter((toneItem) => toneItem.selected)
                      .map((toneItem) => toneItem.tone);
